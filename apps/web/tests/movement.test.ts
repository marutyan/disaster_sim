...
describe("player movement", () => {
  it("uses stable walking-speed calibration independent of keyboard repeat rate", () => {
    ...
    expect(baseWalkingSpeedMetersPerSecond("older_adult", "day")).toBeCloseTo(
      0.53,
    );
  ...
  it("moves toward a map destination without overshooting", () => {
    const partial = moveToward(
      { east: 0, north: 0 },
      { east: 3, north: 4 },
      2,
    );
    expect(partial.east).toBeCloseTo(1.2);
    expect(partial.north).toBeCloseTo(1.6);
    expect(partial.reached).toBe(false);
